...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  if (!args[0]) throw `*[❗𝐈𝐍𝐅𝐎❗] Usa un enlace de Facebook: ${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/`;
  if (!args[0].match(/www.facebook.com|fb.watch/g)) throw `*[❗𝐈𝐍𝐅𝐎❗] 𝙸𝙽𝙶𝚁𝙴𝚂𝙴 𝚄𝙽 𝙴𝙽𝙻𝙰𝙲𝙴 𝙳𝙴 𝙵𝙰𝙲𝙴𝙱𝙾𝙾𝙺, 𝙴𝙹𝙴𝙼𝙿𝙻𝙾: ${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/ /n Si no funciona es posible que el vídeo sea privado o haya sido borrado, tal vez esté en un grupo privado o configurado para amigos.`;
  try {
    await m.reply(`*😇 Ya estoy trabajando con lo que me pediste, solo dame tiempo en lo que termino.*`);
