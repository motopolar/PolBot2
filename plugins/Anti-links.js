...
if (isAdmin && chat.antiLink && m.text.includes(grupo)) return m.reply('*Están prohibidos los links, pero eres admin, perdonado. 😘*')
if (chat.antiLink && isGroupLink && !isAdmin) {
if (isBotAdmin) {
const linkThisGroup = `https://chat.whatsapp.com/${await this.groupInviteCode(m.chat)}`
if (m.text.includes(linkThisGroup)) return !0
}    
await this.sendMessage(m.chat, { text: `*「 𝐀𝐍𝐓𝐈 𝐋𝐈𝐍𝐊𝐒 」*\n*Uy... ${user} Rompiste una regla, lo siento pero adiós.*`, mentions: [m.sender] }) 
if (!isBotAdmin) return m.reply('*[❗𝐈𝐍𝐅𝐎❗] Ey, no soy admin y no puedo eliminar a nadie.*')  
